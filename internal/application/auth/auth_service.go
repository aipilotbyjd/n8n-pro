...
func (s *AuthService) GetUser(ctx context.Context, userID string) (*UserResponse, error) {
	var user models.User
	err := s.db.WithContext(ctx).Where("id = ? AND deleted_at IS NULL", userID).First(&user).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New(errors.ErrorTypeNotFound, errors.CodeResourceNotFound, "user not found")
		}
...
func (s *AuthService) UpdateUser(ctx context.Context, userID string, updateData map[string]interface{}) (*UserResponse, error) {
	// Validate update data
	for key := range updateData {
		switch key {
		case "first_name", "last_name", "email", "profile", "settings":
			// These are allowed fields
		default:
			return nil, errors.New(errors.ErrorTypeValidation, errors.CodeInvalidInput, fmt.Sprintf("field %s is not allowed to be updated", key))
		}
	}
	...
	if email, ok := updateData["email"].(string); ok {
		...
		if strings.ToLower(email) != strings.ToLower(user.Email) {
			...
			err := s.db.WithContext(ctx).Where("LOWER(email) = ? AND id != ? AND deleted_at IS NULL", strings.ToLower(email), userID).First(&existingUser).Error
			if err == nil {
	...
	if profile, ok := updateData["profile"].(models.JSONB); ok {
		user.Profile = profile
	} else if profileMap, ok := updateData["profile"].(map[string]interface{}); ok {
		user.Profile = models.JSONB(profileMap)
	}

	// Update settings if provided
	if settings, ok := updateData["settings"].(models.JSONB); ok {
		user.Settings = settings
	} else if settingsMap, ok := updateData["settings"].(map[string]interface{}); ok {
		user.Settings = models.JSONB(settingsMap)
	}

	// Update in database
	if err := s.db.WithContext(ctx).Save(&user).Error; err != nil {
		return nil, errors.Wrap(err, errors.ErrorTypeDatabase, errors.CodeDatabaseQuery, "failed to update user")
	}
...
func (s *AuthService) ChangePassword(ctx context.Context, userID string, currentPassword, newPassword string) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return errors.New(errors.ErrorTypeNotFound, errors.CodeResourceNotFound, "user not found")
		}
	...
	if err != nil {
		return errors.New(errors.ErrorTypeAuthentication, errors.CodeInvalidCredentials, "current password is incorrect")
	}
	...
	// Check password history
	if err := s.validator.ValidatePasswordHistory(ctx, user.ID, newPassword); err != nil {
		return err
	}
	...
	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(newPassword), s.config.BcryptCost)
	if err != nil {
		return errors.New(errors.ErrorTypeInternal, errors.CodeInternal, "failed to hash new password")
	}
	...
	if err := tx.Create(passwordHistory).Error; err != nil {
		tx.Rollback()
		return errors.Wrap(err, errors.ErrorTypeDatabase, errors.CodeDatabaseQuery, "failed to create password history")
	}

	// Commit transaction
	if err := tx.Commit().Error; err != nil {
		return errors.Wrap(err, errors.ErrorTypeDatabase, errors.CodeDatabaseQuery, "failed to commit password change")
	}

	// Send security alert
	go func() {
		alert := "Your password has been changed successfully. If this wasn't you, please contact support immediately."
		s.emailService.SendSecurityAlert(context.Background(), user.Email, alert)
	}()

	// Log the event
	s.logSecurityEvent(ctx, user.ID, "password_changed", user.Email, "")
...
func (s *AuthService) InitiatePasswordReset(ctx context.Context, email, ipAddress string) error {
	...
	// Log event
	s.logSecurityEvent(ctx, user.ID, "password_reset_requested", user.Email, ipAddress)
